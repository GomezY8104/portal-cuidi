...
export const NewUpaCasePage: React.FC = () => {
  ...
  const handleSubmitTransfer = () => {
    if (!selectedPatient || !clinicalData.mainDiagnosis.code) {
        alert('Dados incompletos. Verifique paciente e diagnóstico.');
        return;
    }

    if (selectedRegulators.length === 0 && regData.priority !== 'VAGA_ZERO') {
        alert('Selecione ao menos uma central de regulação de destino.');
        return;
    }
    ...
    setTimeout(() => {
        ...
        resetUpaFlowData();
        setLoading(false);
        alert(`Solicitação enviada para ${selectedRegulators.length} centrais com sucesso.`);
        navigate('/upa');
    }, 2000);
  };
    ...
    <div className="max-w-[1200px] mx-auto space-y-6 animate-fade-in-up pb-20 font-sans">
                  ...
                  {selectedRegulators.length > 0 && (
                     <div className="flex flex-wrap gap-2">
                        {selectedRegulators.map(id => {
                           ...
                           return (
                              <div key={id} className="px-3 py-1 bg-slate-100 border border-slate-200 rounded-sm text-[9px] font-bold text-slate-600 uppercase flex items-center gap-2">
      ...
      <div className="flex justify-between items-center pt-6 border-t border-slate-200 mt-auto">
         ...
         {step < 5 ? (
           <button 
              onClick={() => {
                  if (step === 1 && !selectedPatient) return alert('Selecione um paciente.');
                  setStep(step + 1);
              }} 
