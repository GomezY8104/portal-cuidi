...
import { HelpCircle, Search, Book, MessageCircle, FileText, ArrowRight, PlayCircle, Home, Mail, Menu, ShieldCheck as ShieldCheckIcon } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="space-y-8">
            <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-700 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest border border-blue-100">
              <HelpCircle size={14} /> Centro de Ajuda
            </div>
            ...
            <p className="text-xl text-slate-500 leading-relaxed max-w-2xl">
              Documentação completa, guias práticos e suporte técnico para a rede CUIDI. Encontre respostas para suas dúvidas sobre a federação.
            </p>
            <div className="max-w-2xl relative mt-8">
            <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400" size={24} />
              <input type="text" placeholder="Pesquisar artigos, políticas ou manuais..." className="w-full pl-16 pr-6 py-5 rounded-[2rem] bg-white border border-slate-200 shadow-lg outline-none focus:shadow-xl focus:border-blue-400 transition-all" />
          </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-16 space-y-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
              { 
                ...
                desc: 'Tudo o que você precisa saber para ativar seu nó.',
                articles: 5
              },
              { 
                title: 'Manuais de Uso', 
                icon: <Book className="text-indigo-500" size={32}/>, 
                desc: 'Guias práticos para profissionais, reguladores e pacientes.',
                articles: 12
              },
              { 
                title: 'Conformidade LGPD', 
                icon: <ShieldCheckIcon className="text-emerald-500" size={32}/>, 
                desc: 'Entenda como protegemos a soberania dos dados.',
                articles: 8
              },
          ].map((item, i) => (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              { q: 'Como criar um nó na federação?', a: 'Você precisa de um token de adesão. Entre em contato com o Ministério da Saúde para obter este identificador único.' },
              { q: 'Como exportar dados dos pacientes?', a: 'Os dados podem ser exportados em formatos padrão FHIR. Consulte o manual de exportação no painel do administrador.' },
              { q: 'Qual é o SLA de disponibilidade?', a: 'A plataforma opera com 99.9% de disponibilidade. Monitore em tempo real no status da rede.' },
              { q: 'Como reportar um incidente de segurança?', a: 'Envie relatórios para security@cuidi.gov.br com detalhes técnicos e evidências do incidente.' },
            ].map((item, i) => (
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-3xl p-12">
          <div className="space-y-6 text-center">
            ...
            <div>
              <h3 className="text-2xl font-black text-slate-900 mb-2">Ainda precisa de ajuda?</h3>
              <p className="text-slate-600 font-medium">Entre em contato com nosso suporte federado disponível 24/7</p>
            </div>
      ...
      <footer className="bg-slate-900 text-white border-t border-slate-800 mt-20">
        <div className="max-w-7xl mx-auto px-6 py-12 text-center text-sm font-medium text-slate-400">
          <p>© 2026 Portal CUIDI - Centro de Ajuda</p>
        </div>
      </footer>
    </div>
  );
